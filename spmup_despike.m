...
% 
% SPM U+ routine to 'despike' fMRI time-series 
% Note the statistics toolbox if required (nansum, icdf are called)
%
...
%             - flags.method defines the despiking method, by default this is
%             set to flags.method = 'median' but can be chaged to 'curve',
%             in that case a smooth curve is fitted to the data folowing
%             the AFNI implementation
...
%             is used to detrermine outliers (see below)
%
% OUTPUT despiked is either the list of despiked images save onto disk or 
%        the despiked data, depending on the input P
%
...
%        - P the list of files (data) analyzed if given or selected
...
%        data folder with the prefix 'despiked_' if no input or P is a series 
%        of names given as input
%
% --------------------------------------------------------------------------
...
% see http://afni.nimh.nih.gov/pub/dist/doc/program_help/3dToutcount.html
%
...
% parameter- see flag options) or using a smooth curve and then we look
% for outliers in the residuals and the data are interpolated as in AFNI.
% see http://afni.nimh.nih.gov/pub/dist/doc/program_help/3dDespike.html
% --------------------------------------------------------------------------
%
% Cyril Pernet 17 Feb 2014
% revision for windowing, and 4D input/output 12 MArch 2014 
% --------------------------------------------------------------------------
...
if get_data == 1;
    ...
    if ischar(P)
        V = spm_vol(P);
        % use the spm_read_vols routine but bypass orientation check
        N = numel(V);
...
% this part is similar to 3dDespike execpt we can use a simple median smoother
            ...
            for z=1:size(Y,3) % smooth the data voxel wise
                ...
                if Mask(x,y,z) ~= 0 && ~isnan(Mask(x,y,z))
                    ...
                    elseif strcmp(flags.method,'curve')
                        ...
                        % TO DO -- iterate per window 
                        % Mini = @(x,t) v - (x(1)+x(2)*t+x(3)*t^2+cumsum(x(4)*sin((2*pi*t)/T)+e*cos((2*pi*t)/T)));
                        % X = fminsearch(@(x) Mini(x,t),X0)
