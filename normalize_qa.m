function normalize_qa(flags,Normalized,T1)
...
if ~isfield(flags,'average'); flags.avergae = 'off'; end
...
elseif nargin < 3
    V = spm_vol(Normalized);
    if spm_check_orientations(V) ~= 1; error('Volumes are not all oriented the same way'); end
    if ~strncmp(V(1).descrip,'spm - 3D normalized',19); warning('Volumes appear not to be SPM normalized ones'); end
    if strcmp(flags.T1,'on')
        T1 = spm_select(1,'image','select normalized T1 image');
...
elseif nargin == 3
    ...
    if ~strncmp(V(1).descrip,'spm - 3D normalized',19); warning('Volumes appear not to be SPM normalized ones'); end
    T = spm_vol(T1);
    if ~strncmp(T.descrip,'spm - 3D normalized',19); warning('The T1 volume appears not to be SPM normalized'); end
end
...
if strcmp(flags.T1, 'on')
    ...
    saveas(gcf,'outlines.eps','psc2'); close(gcf)
