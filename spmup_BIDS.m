...
if isfield(options.preprocess,'despiking')
    ...
    matlabpool % despike uses prarallel processing
    ...
    for s=1:size(BIDS.subjects,2)
        for run = 1:size(BIDS.subjects(s).func,2)
            ...
            in = [options.outdir filesep BIDS.subjects(s).name filesep 'run' num2str(run) ...
                filesep file filesep file];
            spmup_despike(in,[],flags);
        end
    end
