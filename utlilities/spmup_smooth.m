function spmup_smooth(P,Q,s,dtype,M)

% just like spm_smooth with additional mask to mask data before smoothing
% useful when applying brain mask
%
% 3 dimensional convolution of an image
% FORMAT spm_smooth(P,Q,s,dtype,M)
...
% M     - the mask image to apply on P
...
% C Pernet 2020 - added mask argument 
...
if ischar(M), M = spm_vol(M); end

if isstruct(P)
    for i= 1:numel(P)
        if i == 1 && any(P(i).dim ~= M.dim)
            error('data and mask of different dimensions')            
        end
        smooth1(P(i),Q,s,dtype,M);
    end
else
    if any(P.dim ~= M.dim)
        error('data and mask of different dimensions')   
    end
    smooth1(P,Q,s,dtype,M);
end


%==========================================================================
% function smooth1(P,Q,s,dtype,M)
%==========================================================================
function smooth1(P,Q,s,dtype,M)
...
% spm_smooth: spm_conv_vol(P,Q,x,y,z,-[i,j,k]);
% instead pass P as a masked 3D volume 
spm_conv_vol(spm_read_vols(P).*spm_read_vols(M),Q,x,y,z,-[i,j,k]);
