...
### Running multi-echo with TEDANA

SPMUP can also process multi-echo data by calling [TEDANA](https://tedana.readthedocs.io/en/stable/). This means you need to have tedana installed is such a way that the command call works (i.e. from matlab we do `system(tedana_command)`). Specifiy multi-echo in options and that's it, nothing else is required.
