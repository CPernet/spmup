...
% routine to compute the Signal Fluctuation Sensitivity metric
...
elseif nargin < 4
    ...
    roi = [fileparts(which('spmup_sfs.m')) filesep 'external' filesep 'Yeo2011_7Networks_MNI152_FreeSurferConformed1mm_LiberalMask.nii'];
end
...
%% do the computation as in the paper
% 1st data scrubbing without filtering
spm('Defaults','fmri')
spm_jobman('initcfg');
flags.motion_parameters = 'on';
out = spmup_realign_qa(fmridata,flags);
...
matlabbatch{1}.spm.stats.fmri_spec.timing.fmri_t =  Vfmri.dim(3);
matlabbatch{1}.spm.stats.fmri_spec.timing.fmri_t0 = round( Vfmri.dim(3)/2);
